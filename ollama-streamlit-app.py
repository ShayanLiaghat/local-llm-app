...
def stream_chat_local(model, messages):
    try:
        # Initialize the language model with a timeout
        llm = Ollama(model, request_timeout=120)
...
def main():
    ...
    model = st.sidebar.selectbox("Choose a model", ["llama3", "phi", "mistral"])
            ...
            with st.chat_message("assistant"):
                ...
                with st.spinner("Writing..."):
                    try:
                        ...
                        response_message = stream_chat_local(model, messages)
